...
class  Guide {
  ...
  // 导游确认其中一个导游，则给这个选中的导游发送确认行程的模板消息
      public static function Confirm_Gide($aid,$gid,$id,$user_info){
        ...
        global $cfg_guide_appointment;

      $arr_guide = self::Get_Guide_Infromation($gid);

      $name = $arr_guide['name'];   //导游的姓名

      $openid_guide = $user_info['openid_guide'];

    //选中了这个导游之后，则将行程表里面接单的导游的id，姓名，和导游的openid放到行程表里面去,行程改为已确认（state=2）
      $dosql->ExecNoneQuery("UPDATE `#@__travel` set name = '$name', openid_guide='$openid_guide',gid=$gid,state=2 where id=$id");
     ...
     $data=self::send_guide_message($user_info['openid_guide'],$user_info['company'],$user_info['names'],$user_info['tel'],$user_info['title'],$user_info['time'],$user_info['tishi'],$cfg_guide_appointment,$user_info['page'],$user_info['formid_guide']);
      ...
      //删除已经用过的formid
      Common::del_formid($user_info['formid_guide'],$user_info['openid_guide']);
   ...
   public static function get_travel($id){
    ...
    //导游预约成功提醒,给导游发送模板消息
    public static function send_guide_message ($openid,$company,$name,$tel,$title,$time,$tishi,$cfg_guide_appointment,$page,$form_id)
    {
        $data = array(
...
public static  function get_guide_num($id){
  ...
  public function  Send_Concel_Guide($info,$gid,$id){
