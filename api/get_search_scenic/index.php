...
if(isset($token) && $token==$cfg_auth_key){
    ...
    if($i==count($arr)-1){
      $sql1 .= " names like "."'%".$arr[$i]."%'";
    }else{
      $sql1 .= " names like "."'%".$arr[$i]."%'". " or ";
     }
    }

    $sql = "SELECT id,names,label,solds,types,flag,lowmoney,remarks,level,picarr FROM pmw_ticket where checkinfo=1 and ";
      ...
      if($num > 0){  //搜索结果
        ...
        //如果有搜索的内容的时候，则将数据保存到搜索历史表里面
         if(isset($openid)){
           ...
           if(!is_array($r)){
             //判断这个用户的历史搜索记录有几条数据
             $dosql->Execute("SELECT id from pmw_searchlist where openid='$openid' and type=1 order by id desc");
          ...
          //假如搜索历史记录表里面有这个关键字的记录
             ...
             if($nums<=5){
               //删除已经存在的这条关键字的记录
               $dosql->ExecNoneQuery("DELETE from pmw_searchlist where openid='$openid' and type=1 and keyword='$keyword'");
               //保存最新的搜索关键字
               //往搜索历史记录表里面添加搜索记录
               $posttime = time();
               $sql="INSERT INTO  `#@__searchlist` (keyword,openid,posttime,type) values ('$keyword','$openid',$posttime,1)";
